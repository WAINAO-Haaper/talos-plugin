...
assert.match(append, /TALOS 模型执行测试 2026-07-08 23:40/);
assert.match(append, /审批项：#QA-B3 TALOS 模型执行授权测试/);
assert.match(append, /安全边界：本次只写入此目标文件/);
...
assert.match(recorded.content, /\*\*模型执行结果\*\*：✅ 已执行（2026-07-08 23:40，mock-model-file-append）/);
assert.match(recorded.content, /\*\*模型执行目标\*\*：System\/pending-approvals\/model-executor-test\.md/);
assert.match(recorded.content, /\*\*执行结果\*\*：✅ 已批准并完成模型执行测试。/);

console.log("approval executor selftest passed");
