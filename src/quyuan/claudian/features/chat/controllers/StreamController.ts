  ...
  async handleStreamChunk(chunk: StreamChunk, msg: ChatMessage): Promise<void> {
    ...
    switch (chunk.type) {
      ...
      case 'text':
        ...
        (this.deps.plugin as ClaudianPlugin & {
          onQuyuanAssistantText?: (content: string) => void;
        }).onQuyuanAssistantText?.(chunk.content);
        break;

      case 'tool_use': {
        const talosPlugin = this.deps.plugin as ClaudianPlugin & {
          recordQuyuanToolUse?: (toolName: string, input: Record<string, unknown>) => void;
        };
        talosPlugin.recordQuyuanToolUse?.(chunk.name, chunk.input);
        if (state.currentThinkingState) {
          await this.finalizeCurrentThinkingBlock(msg);
        }
        await this.finalizeCurrentTextBlock(msg);
