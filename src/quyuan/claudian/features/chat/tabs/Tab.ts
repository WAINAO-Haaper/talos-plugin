...
type TalosQuyuanPluginBridge = ClaudianPlugin & {
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  ...
  const inputEl = inputWrapper.createEl('textarea', {
    cls: 'claudian-input',
    attr: {
      placeholder: '问屈原，@引用文件、可贴图…（Enter 发送，Shift+Enter 换行）',
      rows: '3',
      dir: 'auto',
    },
  });
...
function initializeInputToolbar(
  ...
  const inputToolbar = dom.inputContainerEl.createDiv({ cls: 'claudian-input-toolbar' });
...
function initializeTalosQuyuanActions(tab: TabData, plugin: ClaudianPlugin): void {
  const bridge = plugin as TalosQuyuanPluginBridge;
  const { dom } = tab;
  if (dom.inputContainerEl.querySelector('.talos-quyuan-action-row')) return;
  const toolbar = dom.inputContainerEl.querySelector('.claudian-input-toolbar');
  const actionRow = dom.inputContainerEl.createDiv({ cls: 'talos-quyuan-action-row' });
  if (toolbar) dom.inputContainerEl.insertBefore(actionRow, toolbar);
  ...
  const actionButton = (
    ...
    const button = actionRow.createEl('button', {
      cls: `talos-quyuan-action ${className}`,
  ...
  actionButton('image-plus', '图像', 'is-image', () => {
  ...
  const micButton = actionButton('mic', '语音', 'is-mic', () => {
    ...
    bridge.toggleQuyuanVoiceInput({
      onInterim: (text) => {
        dom.inputEl.value = text;
      ...
      onStateChange: (listening, error) => {
        micButton.toggleClass('is-active', listening);
        micButton.querySelector('span')?.setText(listening ? '聆听' : '语音');
        if (error) new Notice(`麦克风：${error}`);
      },
    });
  });

  actionButton('square', '停止', 'is-stop', () => {
    tab.controllers.inputController?.cancelStreaming();
    bridge.stopQuyuanVoiceInput?.();
    bridge.stopQuyuanSpeech?.();
  });

  actionButton('send', '发送', 'is-send', () => {
    void tab.controllers.inputController?.sendMessage();
  });

  const voiceParent = (toolbar as HTMLElement | null) ?? dom.inputContainerEl;
  const voiceWrap = voiceParent.createEl('label', { cls: 'talos-quyuan-voice-toggle' });
  voiceWrap.createSpan({ text: '语音' });
  const voiceToggle = voiceWrap.createEl('input', {
    attr: {
      type: 'checkbox',
      'aria-label': '语音助手开关',
      title: '同时控制麦克风输入与回答自动朗读',
    },
  });
  voiceWrap.createSpan({
    cls: 'talos-quyuan-voice-track',
    attr: { 'aria-hidden': 'true' },
  });
  voiceToggle.checked = bridge.getQuyuanVoiceEnabled?.() ?? false;
  voiceToggle.addEventListener('change', () => {
    void bridge.setQuyuanVoiceEnabled?.(voiceToggle.checked);
  });
}
...
export function initializeTabControllers(
  ...
  initializeTalosQuyuanActions(tab, plugin);
}
  ...
  if (tab.service && tab.controllers.inputController) {
    tab.service.setApprovalCallback(
      async (toolName, input, description, options) => {
        const talosPlugin = plugin as ClaudianPlugin & {
          evaluateQuyuanToolPolicy?: (
            toolName: string,
            input: Record<string, unknown>,
          ) => { decision: 'allow' | 'ask' | 'deny'; reason: string };
        };
        const policy = talosPlugin.evaluateQuyuanToolPolicy?.(toolName, input);
        ...
        return await tab.controllers.inputController?.handleApprovalRequest(
          toolName,
          input,
          policy?.reason ? `${description}\n\nTALOS：${policy.reason}` : description,
          options,
        ) ?? 'cancel';
      }
    );
