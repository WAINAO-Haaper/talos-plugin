...
export class TabBar {
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    badgeEl.createSpan({ cls: 'talos-quyuan-tab-index', text: String(item.index) });
    badgeEl.createSpan({
      cls: 'talos-quyuan-tab-title',
      text: this.truncateExpandedTitle(item.title),
    });
    if (item.canClose) {
      const close = badgeEl.createSpan({
        cls: 'talos-quyuan-tab-close',
        text: '\u00D7',
      });
    ...
    badgeEl.setAttribute('data-title-expanded', 'true');
