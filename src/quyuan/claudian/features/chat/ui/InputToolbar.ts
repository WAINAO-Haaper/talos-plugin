...
export class PermissionToggle {
  ...
  getAvailableModes(): ProviderUIOption[] {
    const toggleConfig = this.getToggleConfig();
    if (!toggleConfig) return [];

    const modes: ProviderUIOption[] = [
    ...
    if (toggleConfig.planValue) {
      modes.push({
        value: toggleConfig.planValue,
        label: toggleConfig.planLabel ?? 'PLAN',
      });
    }
    return modes;
  }
  ...
  getCurrentMode(): string {
    return this.callbacks.getSettings().permissionMode;
  }

  async selectMode(mode: string): Promise<void> {
    if (!this.getAvailableModes().some((option) => option.value === mode)) return;
