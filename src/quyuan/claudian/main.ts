...
export default class ClaudianPlugin extends Plugin {
  ...
  async onload() {
    ...
    if (this.shouldRegisterWorkbenchRibbon()) {
      this.addRibbonIcon('bot', '打开屈原 v2 工作台', () => {
    ...
    this.addCommand({
      ...
      editorCallback: async (editor: Editor, ctx) => {
        ...
        const talosPlugin = this as ClaudianPlugin & {
          prepareQuyuanInlineEdit?: (path: string) => Promise<{
            decision: 'allow' | 'deny';
            reason: string;
          }>;
        };
        const policy = await talosPlugin.prepareQuyuanInlineEdit?.(notePath);
    ...
    if (this.shouldRegisterWorkbenchSettingTab()) {
