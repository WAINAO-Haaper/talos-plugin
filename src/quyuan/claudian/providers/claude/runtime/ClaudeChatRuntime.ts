...
export class ClaudianService implements ChatRuntime {
  ...
  closePersistentQuery(_reason?: string, options?: ClosePersistentQueryOptions): void {
    ...
    // Abort as backup.
    // abort() 会同步触发 SDK 的进程关闭链路（曾因浏览器 setTimeout 无 .unref 抛
    // Uncaught TypeError，见 2026-07-09 屈原白屏诊断）。构建层已用 node:timers
    // shim 修根因，这里再兜底隔离，保证清理流程不被打断。
    try {
      this.queryAbortController?.abort();
    } catch (error) {
      console.error('TALOS Claude runtime abort during close failed', error);
    }
