 ...
 *
 * 职责：
 *   1. 配置注册中心 TalosBallRuntime.config：校验 / 深合并默认值 / 导入导出
 *   2. EmotionEngine：rAF 状态机 + 动画原语 + 平滑插值 + 兜底 + 待机策略
 *   3. 对外 SDK：TalosBallRuntime.create(el, opts) → engine 实例
 *
 * 对外 API（宿主工具只依赖这一层）：
 *   const ball = TalosBallRuntime.create(el, { emotion:'02', idle:true });
 *   ball.setEmotion('30');
 ...
 *   TalosBallRuntime.config.exportConfig() / importConfig(json);
 * ============================================================ */
(function () {
  'use strict';

  var EB = (globalThis.TalosBallRuntime = globalThis.TalosBallRuntime || {});
  var RD = globalThis.EB_RINGS;
  ...
  var GROUPS = (globalThis.EMOTION_GROUPS || [
    { key: 'life', name: '生命周期' },
  ...
  function Engine(target, opts) {
    ...
    if (!el) throw new Error('TalosBallRuntime.create：找不到容器元素');
  ...
  Engine.prototype = {
    ...
    setEmotion: function (id, o) {
      ...
      if (!def) {
        console.warn('[TalosBallRuntime] 未知表情 ID "' + id + '"，回退到待机 (' + this._fallbackId + ')');
  ...
  if (Array.isArray(globalThis.EMOTION_SEED)) {
    globalThis.EMOTION_SEED.forEach(function (raw) {
      var r = register(raw);
      if (!r.ok) console.warn('[TalosBallRuntime] 种子配置无效：', r.id, r.errors);
    });
  }
})();
